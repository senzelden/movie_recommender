...
def line(movie_id, title, values="total"):
    ...
    avg_rating = (
        movies_ratings.groupby(["movieId", "year"])[["rating"]]
        .mean()
    ...
    total_ratings["cumsum"] = (
        total_ratings["total_ratings"].groupby(total_ratings["movieId"]).cumsum()
    )
    ...
    if values == "avg":
        ...
        fig.update_layout(
            title=f"{title}: Average Rating over years",
            xaxis_title="Year",
        ...
        fig.update_layout(
            title=f"{title}: Cumulative total ratings over years",
            xaxis_title="Year",
